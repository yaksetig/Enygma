...
async function AuctionInitProof(
    ...
    let wt_commitment;
    if(vaultId == 0){
        wt_commitment = utils.erc20Commitment(wt_contractAddress, wt_idParams[0], keyIn.publicKey);
    }
    else if(vaultId == 1){
        wt_commitment = utils.erc721Commitment(wt_contractAddress, wt_idParams[0], keyIn.publicKey);
    }
    else if(vaultId == 2){
        wt_commitment = utils.erc1155Commitment(wt_contractAddress, wt_idParams[1], wt_idParams[0], keyIn.publicKey);
    }
...
async function AuctionBidProof(
    ...
    for (let i = 0; i < wt_valuesIn.length; i++) {
        let cmt;
            if(st_vaultId == 0){
                cmt = utils.erc20Commitment(assetAddress, wt_idParamsIn[i][0], keysIn[i].publicKey);
            }
            else if(st_vaultId == 1){
                cmt = utils.erc721Commitment(assetAddress, wt_idParamsIn[i][0], keysIn[i].publicKey);
            }
            else if(st_vaultId == 2){
                cmt = utils.erc1155Commitment(assetAddress, wt_idParamsIn[i][1], wt_idParamsIn[i][0], keysIn[i].publicKey);
            }
    ...
    for (let i = 0; i < keysOut.length; i++) {
        let cmt;
            if(st_vaultId == 0){
                cmt = utils.erc20Commitment(assetAddress, wt_idParamsOut[i][0], keysOut[i].publicKey);
            }
            else if(st_vaultId == 1){
                cmt = utils.erc721Commitment(assetAddress, wt_idParamsOut[i][0], keysOut[i].publicKey);
            }
            else if(st_vaultId == 2){
                cmt = utils.erc1155Commitment(assetAddress, wt_idParamsOut[i][1], wt_idParamsOut[i][0], keysOut[i].publicKey);
            }
        st_commitmentsOut.push(cmt);
    }
...
async function BrokerRegistrationProof(
        ...
        for (let i = 0; i < delegator_keys.length; i++) {
                ...
                let cmt;
                if(st_vaultId == 0){
                        cmt = utils.erc20Commitment(wt_contractAddress, wt_delegator_idParams[i][0], delegator_keys[i].publicKey);
                }
                else if(st_vaultId == 1){
                        cmt = utils.erc721Commitment(wt_contractAddress, wt_delegator_idParams[i][0], delegator_keys[i].publicKey);
                }
                else if(st_vaultId == 2){
                        cmt = utils.erc1155Commitment(wt_contractAddress, wt_delegator_idParams[i][1], wt_delegator_idParams[i][0], delegator_keys[i].publicKey);
                }
...
// activate for 10 input joinSplitWithBrokerV1
    ...
    for (let i = 0; i < valuesIn.length; i++) {
        const cmt = utils.erc20Commitment(wt_erc20ContractAddress, wt_valuesIn[i], keysIn[i].publicKey);
    ...
    for (let i = 0; i < keysOut.length; i++) {
        st_commitmentsOut[i] = utils.erc20Commitment(wt_erc20ContractAddress, wt_valuesOut[i], keysOut[i].publicKey);
    }
    wt_pathElements = wt_pathElements.flat(1);
