...
func (c *GnarkClient) Erc1155FungibleWithBrokerV1Proof(
	...
	stCommitmentsOut, wtSaltsOut, ctI, ctII, stNullifiers, wtPathIndices, wtPathElements, err := prepareErc1155ProofParams(
		valuesIn, keysIn, saltsIn, valuesOut, keysOut, recipientViewEncapKeys, merkleDepth, merkleProofs,
	...
	body, err := c.PostProof("/proof/erc1155FungibleWithBroker", payload)
	if err != nil {
	...
	if parseErr := json.Unmarshal(body, &gnarkResp); parseErr != nil {
		return nil, fmt.Errorf("failed to parse erc1155FungibleWithBroker proof response: %w", parseErr)
	}
