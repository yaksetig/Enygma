...
func (c *GnarkClient) Erc1155FungibleWithBrokerV1Proof(
	...
	stCommitmentsOut, wtSaltsOut, stNullifiers, wtPathIndices, wtPathElements, err := prepareErc1155ProofParams(
		valuesIn, keysIn, saltsIn, valuesOut, keysOut, merkleDepth, merkleProofs,
	...
	payload := map[string]interface{}{
		...
		"WtAssetGroupPathElements": bigIntSliceToStrings(assetGroupMerkleProof.Elements),
		"WtAssetGroupPathIndices":  assetGroupMerkleProof.Indices.String(),
		"WtSaltsIn":                bigIntSliceToStrings(saltsIn),
...
// saltsIn must contain the salt used when each input note was originally created.
// Fresh salts are generated for output notes and returned as saltsOut.
func prepareErc1155ProofParams(
	...
	tokenId *big.Int,
) (commitmentsOut []*big.Int, saltsOut []*big.Int, nullifiers []*big.Int, pathIndices []*big.Int, pathElements []*big.Int, err error) {
	...
	for i := 0; i < len(valuesIn); i++ {
		_, err := Erc1155Commitment(contractAddress, tokenId, valuesIn[i], keysIn[i].PublicKey, saltsIn[i])
		if err != nil {
			return nil, nil, nil, nil, nil, fmt.Errorf("failed to compute erc1155Commitment for input %d: %w", i, err)
		}
		...
		if err != nil {
			return nil, nil, nil, nil, nil, fmt.Errorf("failed to compute nullifier for input %d: %w", i, err)
		}
		nullifiers[i] = nullifier
	}

	for i := 0; i < len(keysOut); i++ {
		salt, err := RandomInField()
		if err != nil {
			return nil, nil, nil, nil, nil, fmt.Errorf("failed to generate salt for output %d: %w", i, err)
		}
		saltsOut[i] = salt
		cmt, err := Erc1155Commitment(contractAddress, tokenId, valuesOut[i], keysOut[i].PublicKey, salt)
		if err != nil {
			return nil, nil, nil, nil, nil, fmt.Errorf("failed to compute erc1155Commitment for output %d: %w", i, err)
		}
		commitmentsOut[i] = cmt
	}

	return commitmentsOut, saltsOut, nullifiers, pathIndices, pathElements, nil
}
