...
Private-channel setup visualizes the payer, the chosen recipient, the encrypted request and the relayer's `TagChannelRegistry.openChannel(c1, c2, bitmap)` publication. Multiple directional channels remain listed and selectable. The registry table separates the payer's private recipient choice from the public candidate bitmap: **No privacy**, **Subset**, **Rift**, and **Full privacy** follow the protocol's candidate-set rules. The channel map is a private payer view; the published record exposes the capsule, ciphertext and bitmap.

Each payment explicitly selects an established recipient/channel, one unspent input note, and an amount. The split diagram shows recipient value and payer change, including a zero-valued change output for an exact spend. The sequence shows membership, output construction, Groth16 proving, relay submission, verification, nullifier publication, and two commitment insertions. Spent leaves remain in the append-only tree. The subsequent tag request derives a three-block window and publishes the landing-block tag through `TagRegistry.publishTag()` with a channel-encrypted `amount · token_id · salt` payload. The bitmap belongs to channel setup and is not republished with each payment.
