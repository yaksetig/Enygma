...
The public participant registry remains available on every protocol action screen and always shows each registered name, spend public key, and ML-KEM-768 view public key. Protocol screens read directly from that on-chain registry instead of turning a public-key lookup into a separate transaction or a “retrieve” button.

The Retail walkthrough follows **shield notes → private channels → private payment → recipient scan → chain and wallets → audit access**. The live tree sits alongside shielding and payment controls on desktop, and follows the action on smaller screens. Network traffic is available in these workspaces from the first deposit; there is no trailing traffic step.

The issuer allocates public USD with `RaylsERC20.mint()`. Shielding shows approval, ML-KEM-768/HKDF-SHA256 note construction and `Erc20CoinVault.depositV2()`. Visitors can create one to four notes per submission, repeat deposits with different amounts, and inspect every resulting leaf. The note inventory keeps distinct salts, commitments, values and spent status. Retail commitments use the protocol's four-input Poseidon hash, and the tree uses its depth-8 Poseidon construction and `keccak256("ZkDvp") mod Fr` empty leaf. Upper empty levels are folded in the drawing, while the displayed root includes all eight levels.

Private-channel setup visualizes the initiator, the selected channel peer, the encrypted request and the relayer's `TagChannelRegistry.openChannel(c1, c2, bitmap)` publication. Multiple directional channels remain listed and selectable. The registry table separates the initiator's private channel-peer choice from the public candidate bitmap: **No privacy**, **Subset**, **Rift**, and **Full privacy** follow the protocol's candidate-set rules. The channel map is a private initiator view; the published record exposes the capsule, ciphertext and bitmap.

Channel setup selects a peer for a reusable private channel. Once connected, that peer is marked “Channel established” and cannot be selected again for channel creation. The demo also rejects duplicate requests independently of the UI, including attempts with a different privacy mode. Existing channels remain available for payments after reload and after all peers have been connected. Each subsequent payment selects its recipient using an established channel, one unspent input note, and an amount. The split diagram shows recipient value and payer change, including a zero-valued change output for an exact spend. The sequence shows membership, output construction, Groth16 proving, relay submission, verification, nullifier publication, and two commitment insertions. Spent leaves remain in the append-only tree. The subsequent tag request derives a three-block window and publishes the landing-block tag through `TagRegistry.publishTag()` with a channel-encrypted `amount · token_id · salt` payload. The bitmap belongs to channel setup and is not republished with each payment.

Recipient scanning selects a specific payment, discovers its channel, matches the block tag, opens the channel payload, and recognizes the existing commitment. It adds no leaf. Public chain inspection hides note ownership and values; a selected participant's wallet opens only recognized notes, with unread recipient outputs becoming visible after scanning. Background wallets first obtain deposited notes before spending them, and stopping traffic stops further background insertions. All changes are confined to this folder; contracts, prover and protocol clients remain unchanged.
