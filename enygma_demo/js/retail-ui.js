...
function channelMap(p, preview) {
  ...
  return `<div class="retail-channel-map ${active ? "animating" : ""}" aria-label="Directional private channels from you to recipients"><svg viewBox="0 0 680 ${height}" role="img"><title>${channels.length} established private channels and the selected recipient</title>${recipients.map((id, i) => `<path class="${channels.some(c => c.recipientPartyId === id) ? "established" : "pending"}" d="M 115 ${height / 2} C 295 ${height / 2}, 325 ${(i + .5) * gap}, 483 ${(i + .5) * gap}"/><circle class="channel-packet" r="5"><animateMotion dur="2.5s" repeatCount="indefinite" path="M 115 ${height / 2} C 295 ${height / 2}, 325 ${(i + .5) * gap}, 483 ${(i + .5) * gap}"/></circle><g transform="translate(483,${(i + .5) * gap})"><rect width="185" height="60" x="0" y="-30" rx="12"/><text x="16" y="-4">${nameOf(p, id)}</text><text class="channel-sub" x="16" y="15">${channels.filter(c => c.recipientPartyId === id).length || "New"} ${channels.some(c => c.recipientPartyId === id) ? `established channel${channels.filter(c => c.recipientPartyId === id).length === 1 ? "" : "s"}` : "channel recipient"}</text></g>`).join("")}<g transform="translate(15,${height / 2 - 45})" class="channel-sender"><rect width="125" height="90" rx="16"/><text x="62" y="36" text-anchor="middle">YOU</text><text x="62" y="58" class="channel-sub" text-anchor="middle">Payer’s wallet</text></g></svg><div class="retail-map-caption"><span>PRIVATE WALLET MAP</span><strong>${channels.length} channel${channels.length === 1 ? "" : "s"} established</strong><small>Recipient associations stay in the payer’s wallet.</small></div></div>`;
}
...
export function retailChannelCard(p, modes, registry) {
  ...
  return `<article class="panel flow-card retail-workspace">${head("Private tags · establish channels", "One payer. Multiple private channels.", `${s.channels.length} ESTABLISHED`)}${channelMap(p, draft)}<div class="retail-section-heading"><h3>Create another channel</h3><span>Select a recipient and the public candidate bitmap.</span></div><div class="tag-mode-grid">${modes}</div>${registry}
...
export function retailPaymentCard(p) {
  ...
  return `<article class="panel flow-card retail-workspace">${head("Private payment · payer’s wallet", "Spend a note. Create two new ones.", channel ? `CHANNEL ${channel.index}` : "CHOOSE A CHANNEL")}<div class="retail-live-layout"><div class="retail-action-pane">${walletStrip(p)}
    ...
    <div class="registry-binding-visual retail-binding"><div class="binding-recipient"><small>Selected recipient</small><strong>${recipient?.name || "Choose a channel"}</strong><span>Registry binding</span></div><div class="binding-path"><span><small>pk_spend → commitment</small>${code(recipient?.spendPublicKey)}</span></div><div class="binding-path"><span><small>pk_view → encrypted note</small>${code(recipient?.viewPublicKey)}</span></div></div>
